...
### v1.0.0 — 正式版
- 🎯 正式版发布，代码质量全面优化
- 🔧 修复 v4.0 属性被错误判定为技能的问题
- 🔧 修复序列等级越界（seq_mult 夹紧到 [0,10]）
- 🔧 修复 dictHookList 无 KeyError 保护
- 🔧 修复先攻注册静默吞噬异常
- 🔧 修复别名重复 + 补充 v4.0 属性别名
- 🧹 删除未使用 import（copy/os/json/OlivOS）
- 🧹 提取模块常量消除重复计算
- 🧹 延迟 import 缓存优化高频调用
- 🧹 移除未使用函数（set_runtime_proc/get_runtime_proc）

### v0.2.0 — 自定义回复模板
- ✨ 31 条可编辑自定义回复模板，通过 OlivOS GUI 实时修改
- ✨ 模板覆盖全部回复：属性生成 / D20检定 / SC检定 / 序列刷新 / 先攻 / 错误提示
- ✨ 支持 `{变量}` 占位符：`{nick}` `{d20}` `{total}` `{san}` 等
- ✨ 每条模板附带详细注释（所属指令 + 变量说明 + 效果示例）
- 🔧 合并重复结果行，消除 `= 20` 和 `检定结果: 20` 的重合
- 🔧 删除冗余对抗比较提示行
- 🔧 SC 疯狂阈值 `elif` → 独立 `if`，修复边界漏报
